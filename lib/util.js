...
util.ObjectSearcher.prototype.performSearch = function(object, valueOBj, opt_parentObj) {
  for (var filter in valueOBj) {
    var query = {};
    query[filter] = valueOBj[filter];
        ...
        if (typeof object[objKey] != 'object') {
            if (valueOBj[objKey] == object[objKey]) {
                if (opt_parentObj !== undefined) {
                    // Drop any results not matching the next query
                    for (var i = 0; i < this.results.length; i++) {
                      var result = this.results[i];
                      if(result[objKey] !== undefined) {
                        if(result[objKey] != valueOBj[objKey]) {
                          this.results.pop(result);
                        }
                      }
                    }
